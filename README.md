...
| 特性 | 状态 | 说明 |
|------|------|------|
| ✅ Recall@10 ≥ 85% | **99.30%** | PQ+DiskANN架构 |
...
| ✅ io_uring异步I/O | 已实现 | True batch submit + 拓扑感知预取 |
| ✅ Graph-Aware 2Q缓存 | 已实现 | 图入度感知缓存淘汰，命中率41.2% (LRU仅0.6%) |
...
[Step 7] DiskANN-style search...
  Recall@10:  99.30%  ✓
  QPS:        19.30 (SSD mode, io_uring + BufferPool)
  Cache hit rate: 41.2% (Graph-Aware 2Q)
...
| 缓存命中率 (LRU→2Q) | 0.6% | **41.2%** | - | ↑69x |
| QPS (SSD+io_uring) | 7.29 | **19.30** | - | ↑2.7x |
...
3. **Graph-Aware 2Q缓存**: 图入度感知的2Q淘汰策略，保护hub节点（命中率41.2% vs LRU 0.6%）
